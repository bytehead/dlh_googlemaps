...
class tl_dlh_googlemaps extends \Backend
{
	...
	public function listRecords($arrRow)
	{
		...
		if($arrRow['center'] && $arrRow['zoom'] && $arrRow['mapTypeId']) {
			$src = 'https://maps.google.com/maps/api/staticmap?center=' . $arrRow['center'] . '&zoom=' . ($arrRow['zoom']-2) . '&maptype=' . strtolower($arrRow['mapTypeId']) . '&language=' . $GLOBALS['TL_LANGUAGE'] . '&size=300x150';
